		...
		*/
		...
		function loadpieceoption(page){
			$("#pagechoose tr").empty();
			for(var i=1;i<=widthpiece*heighpiece;i++){
				if(i!=page){
					var eachcolor="<td onclick='changepage("+i+")'>"+i+"</td>";
				}else{
					var eachcolor="<td style='background:red;' onclick='changepage("+i+")'>"+i+"</td>";
				}
				$("#pagechoose tr").append(eachcolor);
			}
		}
		...
		function showcolorchoose(){
			...
			for(var i=0;i<thiscolorCode.length;i++){
				...
				"<td class='colorset' onclick='finish()' style='background-color:"+sHexColor+";'></td>"+
